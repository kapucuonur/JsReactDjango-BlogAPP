...
function App() {
  ...
  const [editArticle, setEditArticle] = useState(null); // Use null instead of an empty string
  const [token, setToken, removeToken] = useCookies(['mytoken']);
  let navigate = useNavigate();

  // Fetch articles when the component mounts or the token changes
  useEffect(() => {
    if (!token['mytoken']) {
    ...
    fetch('http://localhost:8000/api/articles/', {
      ...
      headers: {
        ...
        'Authorization': `Token ${token['mytoken']}` // Use the token from cookies
      }
    })
      .then((resp) => {
        if (!resp.ok) {
